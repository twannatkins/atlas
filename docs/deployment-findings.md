...
| WS1 Module 7 — Bedrock at the Edges | UNBLOCKED | bedrock:InvokeModel AccessDenied on InvokeModel calls. Fixed: added atlas-workshop-bedrock-access inline policy to both execution roles. LIVE-STATE (not in repo). | PURE-LOCAL: no Neptune calls of any kind. SHACL validates in-memory rdflib graphs (g_bad/g_good) against a .ttl file written to disk. Gate is trustworthy. Safe to run top-to-bottom. |
